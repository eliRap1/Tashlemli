...
export async function consumeMagicLink({ token }: ConsumeInput): Promise<{ userId: string }> {
  const tokenHash = createHash("sha256").update(token).digest();
  // Atomically mark the token as consumed. A single UPDATE … RETURNING prevents
  // the TOCTOU window that existed between the previous SELECT and UPDATE pair.
  const [found] = await db
    ...
    .where(
      and(
        ...
        eq(magicLinkTokens.tokenHash as any, tokenHash as any),
      ),
    )
    .returning();
