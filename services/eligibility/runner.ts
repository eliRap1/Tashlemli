...
export async function runJob(jobId: string, fileBytes: Buffer, contentType: string) {
  try {
    ...
    const result = compute({
      ...
      // denied_boarding and rerouted are carrier faults; only "unknown" stays unknown.
      reason_category: ["cancellation", "denied_boarding", "rerouted"].includes(extracted.incident_hint)
        ? "carrier_fault"
        : "unknown",
