...
export async function POST(req: Request) {
  ...
  const strippedType = isImage ? "image/jpeg" : file.type;
  ...
  const blobExt = isImage ? "jpg" : "pdf";
  const blobKey = `eligibility/${sha.slice(0, 2)}/${sha.slice(2, 4)}/${sha}.${blobExt}`;
  const stored = await putPublic(blobKey, stripped, strippedType);
  ...
  await runJob(job.id, stripped, strippedType);
