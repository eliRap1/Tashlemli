...
export async function POST(req: Request) {
  ...
  try {
    ...
    await db.update(eligibilityJobs).set({
      status: "ready",
      flightId: flight.id,
      result,
      extracted: {
        ...(job.extracted as Record<string, unknown>),
        origin_iata: flight.departureIata ?? "TLV",
        destination_iata: flight.arrivalIata ?? "ATH",
      },
    }).where(eq(eligibilityJobs.id, job.id));
