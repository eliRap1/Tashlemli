...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = PasteReplyBody.safeParse(await req.json());
  ...
  await db.insert(claimEvents).values({ claimId, code, actor: "airline", labelHe: cls.summary_he, labelEn: subject, metadata: { manual: true, classification: cls, from, subject } });
  if (cls.requires_lawyer) await db.insert(opsInbox).values({ claimId, kind: code, payload: { from, subject, body, cls } });
  await db.update(claims).set({ currentState: code }).where(eq(claims.id, claimId));
